...
func TestRun_CompletesWeatherWorkflowWithMockProvider(t *testing.T) {
	...
	nodeOutputs, ok := result.Output["node_outputs"].(map[string]map[string]any)
	if !ok {
		t.Fatalf("expected node_outputs payload")
	}
	finalNode := nodeOutputs["finalize_weather_answer"]
	if finalNode["generation_mode"] != "node_llm_generation" {
		t.Fatalf("expected final node to be generated by llm, got %#v", finalNode["generation_mode"])
	}
}
